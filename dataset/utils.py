# 该文件提供了一系列实用函数，主要用于处理视觉问答和图像分割相关的任务。它包括文本预处理、结果评估、结果收集与保存，
# 以及计算 IoU 值的功能。这些工具函数在深度学习模型训练和评估过程中非常重要，有助于提高代码的可读性和可维护性。
# 需要精度
import re
...
def pre_caption(caption, max_words):
    ...
    # 2. 去除多余的空格和换行符
    caption = re.sub(r"\s+", "", caption)  # 去除所有空白字符（包括空格和换行符）
