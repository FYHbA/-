...
def create_dataset(config,test_json=None):
    ...
    if not test_json:
        ...
        test_dataset=DGM4_Dataset(config=config, ann_file=config['val_file'], transform=test_transform, max_words=config['max_words'], is_train=False,test_json=test_json)
        return test_dataset
